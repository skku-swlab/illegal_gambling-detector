...
class HTMLHandler:
    ...
    def __init__(self, wait_time=5, html_save_dir="/home/swlab/Desktop/gambling_crawling/dataset/GNN/crawled_html_files", 
                 graph_save_dir="/home/swlab/Desktop/gambling_crawling/dataset/GNN/gnn_datset"):
        """
        HTMLHandler 초기화
    ...
    def get_full_html(self, url):
        ...
        try:
            ...
            # 자바스크립트 실행 대기
            print(f"{url} 페이지 로딩 중... {self.wait_time}초 대기")
    ...
    def graph_to_json_data(self, graph):
        ...
        for node_id in graph.nodes():
            node_data = {
                ...
                'gambling_score': graph.nodes[node_id].get('gambling_score', 0.0)  # 불법도박 점수 추가
            }
    ...
    def add_gambling_scores_to_graph(self, graph, model_path=None):
        ...
        if model_path is None:
            model_path = "/home/swlab/Desktop/gambling_crawling/Classfication/gambling_bert_model"
    ...
    for url in urls:
        ...
        if file_path:
            ...
            if graph:
                ...
                # 파일명 생성 (원본 HTML 파일명 기반)
                filename = os.path.basename(file_path).replace('.html', '_graph.json')
                # 그래프를 JSON으로 저장
                handler.save_graph_as_json(graph, filename)
